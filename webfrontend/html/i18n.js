...
(function () {
  ...
  var CAT = {
    en: {
      // --- en-Ergaenzungen (Admin-UI: Raum-Panel, Aussehen-Gruppen, Icon-Bibliothek, Geraete) ---
      'Standard-Farbschema': 'Default color scheme',
      'Eine Farbe wählen — Hintergrund, Kacheln, Leiste, Schrift, Icons und Zustandsfarben werden daraus berechnet. Eigene Zustandsfarben behalten Vorrang.': 'Pick one color — background, tiles, bar, text, icons and state colors are derived from it. Custom state colors take precedence.',
      'Eigene Farbe': 'Custom color',
      'Keine': 'None',
      'Standard-Aussehen für alle Panels. Einzelne Panels überschreiben es unter „Darstellung".': 'Default appearance for all panels. Individual panels override it under "Appearance".',
      'Du bearbeitest': 'You are editing',
      'Raum-Panel': 'Room panel',
      'Klassisch': 'Classic',
      'Ein Raum ist die Startseite — das Panel wacht direkt in diesem Raum auf (Licht &amp; Beschattung sofort bedienbar), ohne vorher zu wählen. Ideal für kleine 4″-Panels. Die untere Leiste zeigt automatisch die im Raum vorkommenden Kategorien — ein Tipp scrollt zu den passenden Bausteinen.': 'A room is the start page — the panel wakes up directly in this room (lights &amp; shading usable at once) without choosing first. Ideal for small 4″ panels. The bottom bar automatically shows the categories present in the room — a tap scrolls to the matching devices.',
      'Dieser Raum': 'This room',
      'Kategorie-Tabs (unten)': 'Category tabs (bottom)',
      'Automatisch': 'Automatic',
      'Bis zu 4 Kategorien als untere Tabs. Vorausgewählt sind die ersten im Raum — du kannst sie ändern. „Automatisch" folgt wieder dem Rauminhalt.': 'Up to 4 categories as bottom tabs. The first ones in the room are preselected — you can change them. "Automatic" follows the room contents again.',
      'Bis zu <b>4 Buttons</b> — die 4 Standard-Tabs und/oder einzelne Kategorien (gestrichelt) bzw. Räume (gepunktet). <b>Die Reihenfolge des Anklickens ist die Reihenfolge in der Leiste</b> (1., 2., …), der erste ist die Startseite (★). ': 'Up to <b>4 buttons</b> — the 4 standard tabs and/or individual categories (dashed) or rooms (dotted). <b>The click order is the order in the bar</b> (1., 2., …), the first is the start page (★). ',
      'Aussehen': 'Appearance',
      'Layout': 'Layout',
      'Split / Pane 2': 'Split / Pane 2',
      'Display &amp; Nacht': 'Display &amp; night',
      'Feinjustierung': 'Fine-tuning',
      'Wie eine Kachel im <b>aktiven Zustand</b> hervorgehoben wird (an = Akzent, ok = grün, kritisch = rot): Rahmen, Füllung und Deckkraft. Die <b>Farbe</b> kommt je Zustand aus dem Theme, hier stellst du das <b>Aussehen</b> ein. Darunter der Rahmen der <b>inaktiven</b> Kacheln <b>und der rechten Panes</b> (Pane 2, z. B. Energieflussmonitor, Wetter, Kalender) – Deckkraft und Breite hochdrehen, falls er auf hellen Displays (z. B. Shelly) kaum sichtbar ist. Gilt für dieses Panel — einzelne Kacheln können unten abweichen.': 'How a tile is highlighted in the <b>active state</b> (on = accent, ok = green, critical = red): border, fill and opacity. The <b>color</b> comes per state from the theme; here you set the <b>look</b>. Below it, the border of the <b>inactive</b> tiles <b>and of the right-hand panes</b> (pane 2, e.g. energy-flow monitor, weather, calendar) — turn opacity and width up if it is barely visible on bright displays (e.g. Shelly). Applies to this panel — individual tiles can differ below.',
      ...
      'Keine Loxone-Bibliothek gefunden — LoxoneIcons-Plugin installiert und gemountet?': 'No Loxone library found — LoxoneIcons plugin installed and mounted?',
      ...
      'Fehler:': 'Error:',
      'Das Panel zeigt weiter den Stand von': 'The panel still shows the state from',
      'Uhr.': 'h.',
      ...
      'Standort vom Miniserver wird verwendet.': 'Using the Miniserver location.',
      ...
      'Browser': 'Browser',
      'Ansicht': 'View',
      'Ohne Kennung': 'Without ID',
      'Gerätename': 'Device name',
      'Namen vergeben': 'Assign name',
      'Noch kein Panel gefunden. Ein Panel erscheint, sobald es die Visu mit ?device=<name> öffnet oder der Agent darauf läuft.': 'No panel found yet. A panel appears as soon as it opens the visu with ?device=<name> or the agent runs on it.',
      'Geräte ohne Kennung (nach IP). Einen Namen vergeben, damit das Gerät dauerhaft gelistet und per Betriebsmodus umgeschaltet werden kann.': 'Devices without an ID (by IP). Assign a name so the device stays listed permanently and can be switched via operating mode.',
      ...
      'Noch kein Panel bekannt. Sobald ein Panel die Visu mit Kennung öffnet oder ein Agent läuft, erscheint es hier.': 'No panel known yet. As soon as a panel opens the visu with an ID or an agent runs, it appears here.',
